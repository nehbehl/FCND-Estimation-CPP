...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  float phi_est = rollEst;
  float theta_est = pitchEst;

  Mat3x3F R = Mat3x3F();
  R(0, 0) = 1;
  R(0, 1) = sin(phi_est) * tan(theta_est);
  R(0, 2) = cos(phi_est) * tan(theta_est);
  R(1, 0) = 0;
  R(1, 1) = cos(phi_est);
  R(1, 2) = -sin(phi_est);
  R(2, 0) = 0;
  R(2, 1) = sin(phi_est) / cos(theta_est);  // sec(theta) = 1 / cos(theta)
  R(2, 2) = cos(phi_est) / cos(theta_est);  // sec(theta) = 1 / cos(theta)
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  V3F acc_IMU = attitude.Rotate_BtoI(accel);

  predictedState(3) = curState(3) + acc_IMU.x * dt; // change in velocity along the x is a_x * dt
  predictedState(4) = curState(4) + acc_IMU.y * dt; // change in velocity along the y is a_y * dt
  predictedState(5) = curState(5) + acc_IMU.z * dt - CONST_GRAVITY * dt; // change in velocity along the z is a_z * dt by removing the gravity component
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  RbgPrime(0, 0) = (-(cos(e_theta) * sin(e_psi)));
  RbgPrime(0, 1) = (-(sin(e_phi) * sin(e_theta) * sin(e_psi)) - (cos(e_phi) * cos(e_psi)));
  RbgPrime(0, 2) = (-(cos(e_phi) * sin(e_theta) * sin(e_psi)) + (sin(e_phi) * cos(e_psi)));

  RbgPrime(1, 0) = (cos(e_theta) * cos(e_psi));
  RbgPrime(1, 1) = (sin(e_phi) * sin(e_theta) * cos(e_psi)) - (cos(e_phi) * sin(e_psi));
  RbgPrime(1, 2) = (cos(e_phi) * sin(e_theta) * cos(e_psi)) + (sin(e_phi) * sin(e_psi));
